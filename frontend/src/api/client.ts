...
// Dev-only fixture backend. The module is loaded lazily so it never reaches a
// normal build, and requests made before it finishes simply wait for it.
if (import.meta.env.VITE_MOCK_API === "1") {
	const chargement = import("./mocks");
	apiClient.defaults.adapter = async (config) => (await chargement).mockAdapter(config);
}
